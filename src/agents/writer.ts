...
import { Plan, Evidence, WriterResult, LLMMessage } from '../core/types';
...
export async function runWriter(
  ...
  const span = startSpan('writer', { question: question.slice(0, 100), evidenceCount: evidence.length });
...
Write the complete research report:`,
  ...
  try {
    ...
    endSpan(span, { reportLength: report.length, citationCount: citationMap.size });
